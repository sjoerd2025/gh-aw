...
| Name | Kind | Purpose |
| --- | --- | --- |
| `GH_AW_DEFAULT_OTLP_ENDPOINT` | Variable | OTLP endpoint URL used when no endpoint is configured in frontmatter |
| `GH_AW_DEFAULT_OTLP_HEADERS` | Secret | Exporter headers (`key=value` pairs, comma-separated) sent to that endpoint |

Because both are resolved at run time, setting them at the repository, organization, or enterprise level enables telemetry for every agentic workflow without editing any workflow file.

Behavior:

- When the variable is unset, the endpoint resolves to an empty string and OTLP export is a no-op.
- When the variable is set but `GH_AW_DEFAULT_OTLP_HEADERS` is empty, no span is exported to that endpoint from any job (setup, conclusion, outcome, or MCP gateway) — the runtime treats an endpoint without matching credentials as unconfigured rather than exporting unauthenticated telemetry. The agent job additionally fails with an error so the misconfiguration is visible instead of silently degrading to no telemetry.
...
Set the values with `gh aw env update`, or with `gh variable set` / `gh secret set`.
