...
5. When supported by an agent, AWF creates a trusted `api-proxy` that holds the agent's authentication token. The proxy keeps tokens out of the agent container so that they cannot be exfiltrated in a prompt-injection attack.
