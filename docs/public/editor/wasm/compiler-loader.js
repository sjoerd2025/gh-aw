 ...
 *   const { errors, warnings } = await compiler.validate(markdownString);
 *   // With imports:
...
export function createWorkerCompiler(options = {}) {
  ...
  worker.onmessage = function (event) {
    ...
    switch (msg.type) {
      ...
      case 'error': {
        ...
        if (entry) {
          pending.delete(msg.id);
          // Wrap raw error strings into a structured CompilerError object
          entry.resolve({
            yaml: '',
            warnings: [],
  ...
  /**
   * Validate a markdown workflow string with full schema validation enabled.
   *
   * @param {string} markdown
   * @param {Record<string, string>} [files] - Optional map of file paths to content
   * @returns {Promise<{errors: Array<{field: string, message: string, severity: string}>, warnings: string[]}>}
   */
  function validate(markdown, files) {
    if (isTerminated) {
      return Promise.reject(new Error('Compiler worker has been terminated.'));
    }
    ...
    return new Promise((resolve, reject) => {
      pending.set(id, { resolve, reject });
      const msg = { type: 'validate', id, markdown };
      if (files && Object.keys(files).length > 0) {
        msg.files = files;
      }
      worker.postMessage(msg);
