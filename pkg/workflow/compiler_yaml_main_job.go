...
func (c *Compiler) generateMainJobSteps(yaml *strings.Builder, data *WorkflowData) error {
	...
	// Build a CheckoutManager from the workflow's checkout configuration.
	// When the user specified checkout entries (or checkout: false) we honour them;
	// otherwise the manager falls back to the default single workspace-root checkout.
	checkoutMgr := NewCheckoutManager(data.CheckoutDisabled, data.CheckoutEntries)

	// Determine whether any checkout will be emitted.  This drives later decisions
	// (e.g. where to place runtime setup steps) so we replicate the legacy boolean.
	needsCheckout := !data.CheckoutDisabled

	// When the user supplied custom steps that already contain an actions/checkout
	// step, skip the auto-generated checkout (preserve the legacy skipping behaviour).
	if needsCheckout && data.CustomSteps != "" && ContainsCheckout(data.CustomSteps) {
		compilerYamlLog.Print("Skipping checkout step: custom steps already contain checkout")
		needsCheckout = false
		checkoutMgr = NewCheckoutManager(true, nil) // produce no steps
	}

	compilerYamlLog.Printf("Checkout step(s) needed: %t (%d entries)", needsCheckout, len(checkoutMgr.Entries()))

	// Emit all checkout steps (including dev-mode build steps injected after the first).
