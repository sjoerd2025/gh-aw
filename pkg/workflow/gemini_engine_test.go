...
func TestGeminiEngineExecution(t *testing.T) {
	...
	t.Run("engine env overrides default token expression", func(t *testing.T) {
		...
		assert.NotContains(t, stepContent, "GEMINI_API_KEY: ${{ secrets.GEMINI_API_KEY }}", "Default GEMINI_API_KEY expression should be replaced by engine.env override")
	})
	...
	t.Run("engine env adds extra environment variables", func(t *testing.T) {
		...
		// Custom env var should be added
		assert.Contains(t, stepContent, "CUSTOM_VAR: custom-value", "engine.env should add extra environment variables")
		// Default token should remain when not overridden
		assert.Contains(t, stepContent, "GEMINI_API_KEY: ${{ secrets.GEMINI_API_KEY }}", "Default GEMINI_API_KEY should remain when not overridden")
	})
